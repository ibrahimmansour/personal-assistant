...
│   │   │   ├── nav-customizer.tsx # Which workspaces sit in the bottom nav
│   │   │   ├── workspace-icons.ts # Shared workspace icon-name → component
│   │   │   ├── dashboard-grid.tsx # Widget grid (react-grid-layout)
│   │   │   ├── widget-settings.tsx
│   │   │   └── appearance-picker.tsx
│   │   ├── views/                 # status-board, today, inbox, timeline, item-detail
...
│   │   ├── use-back-handler.ts    # History-backed dismissal (system back gesture)
...
`workspace-context.tsx` owns workspaces (persisted per profile). A workspace either renders the widget grid (default) or a dedicated view via `viewType`: `status-board`, `today`, `inbox`, `timeline`. Built-in workspaces are marked `builtIn: true`; horizontal swipe on `<main>` cycles them. `status-board` is the default active workspace, and the provider repairs saved state that predates it.

The mobile bottom nav is **user-chosen, not the first five built-ins**: `navWorkspaceIds` (part of `WorkspaceState`, so it persists per profile through the same localStorage + `/api/dashboard` path) lists up to `NAV_SLOTS` = 5 workspace IDs in display order, defaulting to `status-board`, `dashboard`, `today`, `inbox`, `timeline`. `NavCustomizer` edits it — long-press the nav strip, or the plain buttons on the mobile launcher and in the sidebar drawer, all of which dispatch the `open-nav-customizer` window event (the same idiom as `widget-reorder-mode`). Saved state that predates the field falls back to the default, IDs are re-checked against the live workspace list on load, and `deleteWorkspace` drops the deleted ID from the nav so no tab points at nothing. Custom workspaces are eligible too, so the nav is not limited to `builtIn` ones.

The `today`, `inbox` and `timeline` views share one detail pane, `views/item-detail.tsx` (`ItemDetail`), covering all five item kinds (email, PR, Jira, calendar, task). It owns the email-body fetch, because **the list endpoints do not return message bodies above 50 messages**: `/api/outlook/emails` and `/api/google/emails` switch to a light projection at `limit > 50` that drops `Body`/`bodyHtml` entirely, so anything reading a body has to re-fetch the message by ID from `/api/{outlook,google}/emails/[id]`. The email widget does the same in `selectEmail()` and prefers its fetched record over the list row it shadows — reading the list row instead is what made a tapped email render as plain preview text rather than HTML. Don't render an email body straight from a list row.
...
**The system back gesture is wired through `use-back-handler.ts`, not the router.** The app is a single page with a single history entry, so on Android (and in an installed PWA) back used to walk straight out of the app even with an expanded widget covering the screen. Every dismissible surface registers a *layer*: `useBackHandler(active, onBack)` pushes a history entry when `active` turns true and routes the resulting `popstate` to `onBack`; closing by any other means (X button, Escape, swipe-to-dismiss) unwinds the entry in the effect cleanup via `history.go()`, marked as ours so the listener doesn't dismiss a second layer. The stack is module-level and LIFO, so nesting unwinds in order. Registered today: the expanded-widget overlay, the AI panel, the mobile sidebar drawer, the command palette, `ItemDetail`, the nav customizer, and workspace switches (via the `pushBackLayer` primitive — each switch earns its own entry the way a page navigation would, minus hydration and profile swaps). Two rules for new surfaces: give a full-viewport surface a back layer alongside its Escape handler, and **do not remount a component that holds a layer** (no `key={id}` that changes on selection) — the cleanup's `history.go()` and the remount's `pushState` race, and the entry count drifts.
...
The `claude-code` widget is **background-only below 768px**: mode is pinned to `"background"` regardless of the persisted `claude-code-mode`, and the mode toggle (header + folder picker) and the Chat/Terminal switch are hidden. Its session sidebar is a full-width `absolute inset-0` overlay on mobile (a 240px pane would squeeze the main pane's toggle out of reach) with its own close button, and it collapses itself after New Session or picking a session. The folder picker renders in the *main* pane, underneath that overlay, so opening it goes through `openFolderPicker()`, which closes the sidebar on mobile first ("Browse…" and "New Session" used to open it invisibly behind the session list), and `closeFolderPicker()` hands back to the sidebar when no session is active. Both mobile surfaces hold a `useBackHandler` layer (picker; sidebar only while a session is behind it). On a phone the picker scrolls as a whole, Recent is a horizontal chip strip rather than ten 44px rows, and the browser hides dot-directories — otherwise the home directory is a screenful of tool caches per level.

**`WidgetWrapper`'s `sidePanel` is desktop-only** — it renders `hidden md:block`, so a widget that puts its detail view there has *no* detail view on a phone. That is what broke the news reader: tapping an article expanded the widget and left the same list on screen. The news widget now branches on `useIsMobile()` — it passes `sidePanel` only above `md`, and below `md` renders `ReaderPane` (with `fullWidth`, dropping the border/rounding that read as a separate pane) as the widget *body* in place of the list. Because that makes it a dismissible full-viewport surface it takes its own `useBackHandler` layer, stacked above `WidgetWrapper`'s, so the first back closes the article and the second closes the widget. Any widget adopting `sidePanel` owes the same mobile branch.
