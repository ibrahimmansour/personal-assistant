...
### Mobile / PWA
Installable PWA (`public/manifest.webmanifest`, icons, `appleWebApp` metadata, `viewportFit: "cover"`). Mobile surface: `MobileBottomNav`, swipe gestures via `use-swipe` (80px threshold, 40px edge exclusion reserved for sidebar/AI-panel edge swipes), and safe-area utilities in `globals.css`. Pinch-zoom is deliberately left enabled for accessibility — do not re-lock `maximumScale`. Full-viewport touch surfaces (expanded-widget overlay, sidebar drawer, AI panel, inbox detail pane) restrict `touch-action` to `pan-y` so a custom swipe-to-dismiss gesture doesn't fight the browser's native scroll — each of these also carries `touch-pinch-zoom` alongside `pan-y` so that restriction doesn't also swallow two-finger pinch-zoom (which, since it's an optical scale of the rendered page, always scales text along with everything else — there's no separate "zoom the fonts" toggle needed once pinch-zoom itself isn't blocked). A new full-viewport swipeable surface should follow the same `touch-pan-y touch-pinch-zoom` pairing. CSS alone is not enough, though: the gesture hooks in `use-swipe.ts` share a window-wide capture-phase pointer tracker and every one of them (`useEdgeSwipe`, `useSwipe`, `useLongPress`) aborts the in-flight gesture the moment a second pointer goes down, and each ignores pointer events whose `pointerId` isn't the one that started the gesture. Without that, a pinch read as a one-finger swipe — the expanded card dragged away and dismissed instead of zooming, and an edge-started pinch hit `preventDefault()` in `useEdgeSwipe`, cancelling the zoom outright. Any new pointer-driven gesture must take the same guard. None of that helps in an **installed PWA**, though: Chrome's WebAPK and iOS home-screen apps (`display-mode: standalone` / `navigator.standalone`) disable the browser's page zoom outright, so the viewport meta and the `touch-action` pairings have nothing left to enable. `usePinchZoom` (`src/hooks/use-pinch-zoom.ts`, mounted via `PinchZoomLayer` in `page.tsx`) covers that case by driving CSS `zoom` on `<html>` (class `html.app-zoom`, variable `--app-zoom`, persisted in `localStorage` under `pa-app-zoom`) from a two-finger gesture, clamped to 0.8–3×. It is inert in a browser tab — running both would compound the two scales. `zoom` rather than `transform: scale` so the layout reflows and no pan affordance is needed; distances come from `screenX`/`screenY` because `clientX`/`clientY` are CSS pixels of the document we are actively zooming. The zoom-out floor is 0.8 because the mobile/desktop split is a 768px query against the *zoomed* viewport width — zoom out far enough and a phone flips to the desktop grid mid-gesture.

Because zoom divides the layout viewport (a 390px phone lays out at 130 CSS px at 3×) and the font-size setting widens every string without adding room, both scale knobs expose the same failure: content that can only be laid out at its min-content width pushes past the screen edge and reading costs a sideways scroll. Two global defaults in `globals.css` hold the line — `body { overflow-wrap: anywhere }` and `body * { min-width: 0 }`. `anywhere` rather than Tailwind's `break-words` (`overflow-wrap: break-word`) because only `anywhere` shrinks the element's min-content contribution, and `min-width: 0` overrides the `auto` (= min-content) automatic minimum size of flex/grid children, the other half of the same blowout — most visibly under a `truncate`d label. Explicit `min-w-*` utilities outrank the universal selector and `shrink-0` children keep their natural width, so deliberate horizontal strips (mobile Recent row, files-widget column browser) still work. On top of that, `WidgetWrapper`'s `CardContent` and the status-board grid scroll `overflow-y-auto overflow-x-hidden`, so overflowing widget content wraps instead of offering a sideways scroll; a widget that genuinely needs one (tables, code, terminal) owns an inner `overflow-x-auto` scroller. The email iframe carries the same `overflow-wrap: anywhere` in its `srcdoc` styles. When adding a surface, don't reintroduce plain `overflow-auto` at the widget-frame level.
...
Mobile UX baseline (audited against the Vercel Web Interface Guidelines; the skill lives at `.claude/skills/web-design-guidelines/` and fetches its rules fresh on each run). Four global rules in `globals.css` back it, and new surfaces inherit them rather than restating them:
...
- Mobile `input`/`textarea`/`select` render at `max(16px, 1rem)`. Under 16px iOS Safari auto-zooms the viewport on focus and leaves it zoomed on blur, and the appearance font-size setting (`xs` = 13.6px root) drops every rem-sized control under that line. The selector's `:not()` clauses deliberately outrank a single Tailwind `text-*` utility.
- `button, a, [role="button"], label, summary` get `touch-action: manipulation` (drops the double-tap-zoom wait; pan and pinch are unaffected, so the accessibility zoom above still works) and `-webkit-tap-highlight-color: transparent`. Because that kills the UA's grey flash, **a new control needs its own `active:` state** or a tap reads as dead.
